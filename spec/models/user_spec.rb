  ...
  describe 'ユーザー新規登録' do
    ...
    context '新規登録がうまくかないとき' do
      ...
      it '姓（カナ）が空だと登録できない' do
        ...
        @user.valid?
        expect(@user.errors.full_messages).to include("名字フリガナを入力してください", "名字フリガナは不正な値です")
      end
      it '名（カナ）が空だと登録できない' do
        @user.first_name_kana = ''
        @user.valid?
        expect(@user.errors.full_messages).to include("名前フリガナを入力してください", "名前フリガナは不正な値です")
      end
      it 'パスワードが数字のみの場合保存できない' do
        @user.password = '123456'
        @user.password_confirmation = '123456'
        @user.valid?
        expect(@user.errors.full_messages).to include("パスワードは半角英数字の両方で入力してください")
      end
